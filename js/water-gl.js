...
"void main(){",
"  float fl     = 1.0 - uDrainLevel;",
"  float inWater = step(fl, uv.y);",
...
"  float aboveW  = 1.0 - step(uWaterlineY, p.y);",
"  return edge * aboveW * uPourActive;",
"}",
"void main(){",
"  float waterlineY = 1.0 - uDrainLevel;",
...
function addDrop(clientX, clientY, strength) {
  drops.push({ x: clientX/W, y: 1-clientY/H, r: 0.045, str: strength||0.6 });
}
...
addEventListener("pointermove", function(e){
  ...
  if(dx*dx+dy*dy>144){ addDrop(e.clientX,e.clientY,0.18); lastMX=e.clientX; lastMY=e.clientY; }
...
addEventListener("pointerdown", function(e){
  ...
  glowEl.style.width="480px"; glowEl.style.height="480px";
  showGlow(e.clientX,e.clientY);
  setTimeout(function(){
    glowEl.style.transition="opacity 0.4s, width 0.3s, height 0.3s";
    glowEl.style.width="320px"; glowEl.style.height="320px";
  },120);
});

setInterval(function(){
  document.querySelectorAll(".reflect").forEach(function(r){
    var ox=(Math.random()-.5)*5;
